...
if(isset($_GET['action'])) {
	if($_GET['action']=="create") {
		...
		$jsondata = json_encode($data);
		file_put_contents($filename, $jsondata);
		header("location:forminput.php?msg=success");
	}
...
function printUser($user){
	...
	<form class="user" action="forminput.php?action=<?= $action?>&id=<?= $_GET['id']?>" method="post">
		<div class="user-name">
			<label>Name</label>
...
<body>
	...
	if(isset($_GET['id'])) {
		...
		$user = $_GET['id']=="new" ?
			...
			$data->users[$_GET['id']];
		printUser($user);
		echo "<a href='forminput.php'>Back</a>";
	} else {
		// Show the user list
		echo "<a href='?id=new'>Add New User</a><hr>";
		foreach($data->users as $id=>$value) {
			// template for PHP (a repeated batch of code)
			echo "      
			<div class='user'>
